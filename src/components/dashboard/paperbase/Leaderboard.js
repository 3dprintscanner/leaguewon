...
import { useFollowing } from "./../../../util/db";
...
function Leaderboard(props) {
    ...
    const mapItem = (it, idx) => {
        console.log(it, idx)
        const { id, name } = it.following
        const pl = getRandomInt(-5000, 5000)
        const plPc = getRandomPercent();
        return (<ListItem button component={Link} to={`/traders/${id}?pl=${pl}`}>
            ...
            <ListItemText primary={renderPlPc(plPc)} />
    ...
    return (
        <Paper className={classes.paper}>
            ...
            {(items && items.followers && mapItems(items.followers)) ?? (<div className={classes.contentWrapper}>
